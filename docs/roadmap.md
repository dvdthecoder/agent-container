...
| [#112](https://github.com/dvdthecoder/agent-container/issues/112) | Parse structured JSON events from opencode into SQLite (thinking token tracking) | ✅ `think_chars` in `runs`; structured `tool_call` emit; `GET /runs/{id}/turns`; `logs --turns` |
